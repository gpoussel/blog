...
- `getPublishedPosts()` — newest-first, excludes `draft: true` in production. Used by the home page, `blog/[...slug].astro` (`getStaticPaths`), and `rss.xml.js`. Everything (lists, category pages, counts) must keep flowing through it so drafts never leak.
