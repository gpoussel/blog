...
- `getPublishedPosts()` — newest-first, excludes `draft: true` **and future-dated posts** in production (day-granular UTC comparison via `src/utils/publication.ts`, shared with the deploy gate script so the two notions of "published" cannot drift; `pnpm dev` shows everything). Used by the home page, `blog/[...slug].astro` (`getStaticPaths`), and `rss.xml.js`. Everything (lists, category pages, counts) must keep flowing through it so drafts and scheduled posts never leak.
- **Categories.** A post has one or more free-form `categories` (frontmatter array, min 1), normalized on read: `postCategories(post)` (trimmed, de-duplicated), `getCategories()` (unique categories + counts), `getPostsByCategory()`, and `slugify()` — a category's _identity_ is its slug, so "Code golf" and "code golf" are one category and `/category/<slug>/` URLs stay stable.
- `PAGE_SIZE` (8) — shared page size for the home and category lists.
- `readingTime()` and `formatDate()` (locale `en-US`) for post metadata.

**Pages** (`src/pages/`): `[...page].astro` (the paginated home list: `/`, `/2`, …), `category/[category]/[...page].astro` (one paginated list per category), `about.astro`, `blog/[...slug].astro`, `rss.xml.js`, and `og/[...route].png.ts` (generated social images, below). All HTML pages wrap `layouts/BaseLayout.astro`, which sets `<head>`, SEO/OG tags, the canonical URL, and (for paginated lists, via the `prev`/`next` props) `<link rel="prev"/"next">`. The `site` field in `astro.config.mjs` (`https://gpoussel.fr`) feeds canonical URLs, RSS, the sitemap, and the OG image URLs/footer host.

**Lists paginate at `PAGE_SIZE` via Astro's `paginate()`.** The featured-card rule: home page 1 shows the newest post as a featured card and paginates the _rest_ (so page 1 = featured + up to `PAGE_SIZE` cards, later pages exactly `PAGE_SIZE`); category pages have no featured card. Category pills are links (one per category) on post cards and post headers; `components/Pagination.astro` is the shared pager (hidden when there is a single page). In `PostCard.astro` the title link is stretched over the card with `::after` so the pills stay independently clickable — don't nest links inside it.
...
**Auto-deploy is GitHub Actions, defined in `.github/workflows/deploy.yml`.** On every push to `main` (and via manual `workflow_dispatch`) it builds with pnpm and uploads `dist/` to the server with `rsync` over SSH (auth via `sshpass`, no third-party deploy action). It also runs on a **daily schedule (06:30 UTC)** so future-dated posts go live on their `pubDate` without manual action: a cheap `gate` job (no build) runs `scripts/scheduled-deploy-gate.ts`, which compares the set of published slugs at HEAD/today against the set at the last successful run's commit/start time (frontmatter read via `git show`, parsed with the same `postSchema` + `src/utils/publication.ts` predicate as the site), and the expensive `deploy` job only runs when the sets differ. Push and manual runs bypass the comparison (always deploy); the gate's `github.ref` check ensures nothing deploys off `main`. The workflow is deliberately hardened for a public repo, preserve all of this if you touch it:
